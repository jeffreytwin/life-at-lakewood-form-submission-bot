...
import { emptySiteCounts, listOpenErrors, siteCounts } from "@/lib/listings/hub";
 ...
 *
 * What the Hub's Listings section shows: the engine switch and state,
 * each site with its listing counts by state, the newest runs, and the
 * error events nobody has dismissed yet.
 */
export async function GET(request: NextRequest) {
  const denied = authorizeEngineRequest(request);
  if (denied) return denied;
  try {
    const [{ data: settings }, { data: sites }, { data: runs }, openErrors, perSite, villages] = await Promise.all([
    ...
    return NextResponse.json({
      ...
      runs: runs ?? [],
      errors: openErrors.errors,
      openErrors: openErrors.count,
    });
  } catch (error) {
