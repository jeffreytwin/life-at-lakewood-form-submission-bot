import { NextRequest, NextResponse } from "next/server";
import { logger } from "@/lib/shared/logger";
import { errorMessage } from "@/lib/shared/errors";
import { authorizeEngineRequest } from "@/lib/listings/auth";
import { loadActiveSites } from "@/lib/listings/db";
import { seedSiteMediaFromLive } from "@/lib/listings/media-seed";

export const dynamic = "force-dynamic";
export const maxDuration = 300;

/**
 * POST /api/internal/listings/media/seed
 * Body: { siteId?: string }
 *
 * Reads each active site's live galleries (read-only) and records the
 * photos it already serves, so the engine reuses them instead of
 * downloading them again from MLSGrid.
 */
export async function POST(request: NextRequest) {
  const denied = authorizeEngineRequest(request);
  if (denied) return denied;
  let siteId: string | undefined;
  try {
    const body = (await request.json()) ?? {};
    if (typeof body.siteId === "string") siteId = body.siteId;
  } catch {
    // no body
  }
  try {
    const sites = await loadActiveSites(siteId ? [siteId] : undefined);
    const results = [];
    for (const site of sites) {
      if (!site.wix_site_id) {
        results.push({ domain: site.domain, skipped: "no wix_site_id" });
        continue;
      }
      results.push({ domain: site.domain, ...(await seedSiteMediaFromLive(site)) });
    }
    return NextResponse.json({ sites: results });
  } catch (error) {
    logger.error("Media seed failed", { error: errorMessage(error) });
    return NextResponse.json({ error: errorMessage(error) }, { status: 500 });
  }
}
