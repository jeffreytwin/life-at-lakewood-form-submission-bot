...
/**
 * Re-sync specific messages by Gmail search query for a given account.
 * Bypasses the History API cursor — useful for replaying messages that
 * were missed due to misconfiguration.
 */
export async function resyncByQuery(
  accountEmail: string,
  query: string,
  maxResults = 10
): Promise<{
  ...
  const { data: accounts, error } = await supabase
    .from("email_accounts")
    .select("*")
    .ilike("email_address", accountEmail)
    .eq("is_active", true)
    .limit(1);

  if (error) throw new Error(`Failed to load account: ${error.message}`);
  if (!accounts || accounts.length === 0) {
    throw new Error(`No active account found for ${accountEmail}`);
  }

  const account = accounts[0] as EmailAccount;
  const creds = account.credentials as GmailCredentials | null;
  if (!creds) throw new Error(`No credentials for ${accountEmail}`);

  const messageIds = await listMessages(account.id, creds, query, maxResults);
  ...
  for (const ref of messageIds) {
    try {
      const msg = await getMessage(account.id, creds, ref.id);
      const result = await processInboundMessage(account, msg);
      // If the upsert detected a duplicate, both flags will be false but
      // the message wasn't truly "new" — check the DB to distinguish.
      if (!result.draftGenerated && !result.skippedNonLead) {
        // Could be a duplicate or an outbound message — check
      ...
      logger.error("Resync: failed to process message", {
        accountId: account.id,
        messageId: ref.id,
        error: err instanceof Error ? err.message : String(err),
      });
  ...
  return { newMessages, draftsGenerated, skippedNonLead, alreadyExists };
}
